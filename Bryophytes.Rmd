...
betamat[!cell8, !cell8, drop=TRUE]

inx <- match(as.character(cell8), rownames(betamat))

betamat <- betamat[inx,inx]
```
...
row.names(betamat) <- cell8
names(betamat)[1:3788] <- cell8
...
```{r}
mean(full.betamat["767", as.character(bry8neighbors[,"767"])])

cell8_ch <- as.character(cell8)

ultimatebeta <- lapply(cell8_ch, function(x)mean(full.betamat[x, as.character(bry8neighbors[,x])]))
names(ultimatebeta) <- cell8_ch
plot(cell8_ch, neighbormats)
...
#Plot richness
```{r}
betavector<-unlist(ultimatebeta)
...
BetaRaster[which(betavector==0)]=NA
plot(BetaRaster, axes = F)
```

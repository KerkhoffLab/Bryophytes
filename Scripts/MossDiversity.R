...
MossBetaMat7[!cell7, !cell7, drop=TRUE]
...
readRDS(MossBetaMat, file = "Data/MossBetaMat.rds")

bry8neighbors <-neighbors[cell8]
bry7neighbors <- neighbors[cell7]
bry8neighbors <- data.frame(bry8neighbors)
names(bry8neighbors)[1:3782] <- cell8
bry7neighbors <- data.frame(bry7neighbors)
names(bry7neighbors)[1:321] <- cell7
row.names(MossBetaMat8) <- cell8
names(MossBetaMat8)[1:3782] <- cell8
...
ultimatebeta8 <- lapply(cell8_ch, function(x)mean(full.MossBetaMat[x, as.character(bry8neighbors[,x])]))
names(ultimatebeta8) <- cell8_ch
ultimatebeta7 <- lapply(cell7_ch, function(x)mean(full.MossBetaMat[x, as.character(bry7neighbors[,x])]))
